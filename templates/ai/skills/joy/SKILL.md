...
2. Run `joy ai update --check` to verify AI instructions are current. If it exits with code 2, tell the user which templates are outdated and suggest `joy ai update`. Do not proceed with outdated instructions.
3. Run `joy config get modes.default` to read the mode. If the key does not exist, default to `collaborative`.
