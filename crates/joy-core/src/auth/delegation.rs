...
/// Derive a 32-byte Ed25519 seed for a per-(human, AI) delegation key
/// (ADR-037, updated by ADR-039).
///
/// Inputs:
///   - `identity_seed`: the human's 32-byte identity seed (the value
///     unwrapped from `seed_wrap_passphrase`, which under ADR-039 stays
///     stable across passphrase rotation). On legacy projects without
///     a wrap, callers pass the bytes of the Argon2id-derived key
///     directly, since that value equals the seed pre-migration.
