...
pub fn run(args: InitArgs) -> Result<()> {
    ...
    // Who is behind this process, decided once, here (D1.1): a person at a
    // terminal may be asked for the founding address; a hook, a pipe, a
    // --json run and a delegated agent are refused by name instead
    // (D3.9). `--json` is not a terminal for this purpose: its stdout
    // carries one envelope and nothing else.
    let host = joy_core::host::HostKind::detect(
        !crate::output::is_json() && crate::prompt::is_interactive(),
    );
