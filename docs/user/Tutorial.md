...
Both `bump` and `record` take an optional prerelease label (e.g. `alpha`, `beta`, `rc1`) as a second argument or via `--prerelease`/`--pre`: `joy release bump minor alpha` turns `v0.4.0` into `v0.5.0-alpha`, and bumping the same label again turns it into `v0.5.0-alpha.1`. A keyword bump (`patch`/`minor`/`major`) from a prerelease base with no label graduates it to the matching stable version instead of skipping or going backwards (`v0.30.0-beta` + `patch` -> `v0.30.0`). Releases sort by SemVer 2.0 precedence, so a stable version always sorts above its own prereleases, and recording the final stable release rolls up every item closed since the *first* prerelease of that cycle, not just since the last one.
