...
│   └── ...
├── ai/                      # Generated by joy ai init (gitignored)
...
External AI agents use Joy's CLI as a tool. The agent calls `joy` commands to read, create, and manage items. `joy ai init` checks project documentation, installs AI instructions and skills into `.joy/ai/`, detects installed AI tools, and generates the appropriate config files with permissions.
...
```sh
joy ai init                            # Set up AI tool integration (MS-02)
...
- `joy ai init` - check docs, install AI instructions/skills, configure detected tools
- `joy config` - show current configuration
...
- `joy ai init agent` - configure AI tool and model
